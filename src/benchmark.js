...
let M_thin = allocate(1000000).map(() => random(3))
let M_eq = allocate(1000000).map(() => 7)
...
let impl_M_thin = make_sort_benchmark('impl_M_thin', M_thin, intsort).run()
let impl_M_eq = make_sort_benchmark('impl_M_eq', M_eq, intsort).run()
...
let control_M_thin = make_sort_benchmark('control_M_thin', M_thin, inplace_nativesort).run()
let control_M_eq = make_sort_benchmark('control_M_eq', M_eq, inplace_nativesort).run()
